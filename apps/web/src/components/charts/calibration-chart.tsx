    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span className={`text-sm px-2 py-1 rounded ${isWellCalibrated ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
            Brier: {brier.toFixed(3)}
          </span>
        </CardTitle>
        <CardDescription>
          Predicted vs actual outcomes. Perfect calibration follows the diagonal line.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="h-80">
                  ...
                  if (active && payload && payload.length && payload[0]?.payload) {
                    ...
                    return (
                      <div className="bg-background border rounded-lg p-3 shadow-lg">
                        <p className="font-medium">Confidence: {data.predicted?.toFixed(1) || '0.0'}%</p>
                        <p className="text-sm text-muted-foreground">
                          Actual: {data.actual?.toFixed(1) || '0.0'}%
                        </p>
                        <p className="text-sm text-muted-foreground">
                          Difference: {data.actual && data.predicted ? (data.actual - data.predicted).toFixed(1) : '0.0'}%
                        </p>
                      </div>
                    );
                  }
        ...
        <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <p className={`font-medium ${isWellCalibrated ? 'text-green-600' : 'text-yellow-600'}`}>
          ...
          <div>
            ...
            <p className="font-medium">
              {agent.calibration.some(p => p.predicted > p.actual + 0.05) ? 'Detected' : 'None'}
            </p>
          </div>
          <div>
            <p className="text-muted-foreground">Sample Size</p>
            <p className="font-medium">{agent.calibration.length * 20} forecasts</p>
          </div>
        </div>
      </CardContent>
    </Card>
